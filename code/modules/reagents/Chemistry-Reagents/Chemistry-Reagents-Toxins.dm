...
/datum/reagent/three_eye/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	...
	if(prob(5))
		to_chat(M, ("<font size = [rand(1,3)]>[pick(dose_messages)]</font>"))

/datum/reagent/three_eye/overdose(var/mob/living/carbon/M, var/alien)
	..()
	M.adjustBrainLoss(rand(1, 5))
	if(ishuman(M) && prob(70))
		var/mob/living/carbon/human/H = M
		H.make_jittery(20)
		H.make_dizzy(12)
		H.adjustBrainLoss(rand(8, 12))
	if(prob(10))
		to_chat(M, ("<font size = [rand(2,4)]>[pick(overdose_messages)]</font>"))
	if(M.psi)
...
/obj/item/weapon/reagent_containers/pill/three_eye/Initialize()
	. = ..()
	reagents.add_reagent(/datum/reagent/three_eye, rand(10,20))
