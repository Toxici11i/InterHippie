...
/obj/machinery/power/nuclear_rod/proc/check_state()   // Well, this is kinda ugly, but at least it works
	...
	if (integrity <= 0 && broken == 0)
		...
		radiation_repository.radiate(src, 500)
		sealed = 0
		broken = 1
		reactants = list()
...
/obj/machinery/power/nuclear_rod/process()     // Here is main purpouse of the rod - heating and radiating.
	...
	if(sealed == 0)
		...
		radiation_repository.radiate(src, emitted)
		...
		radiation_repository.radiate(src, round (own_rads * sealcoeff))
...
/obj/machinery/power/nuclear_rod/proc/React() //This proc is quite baggy, so not be suprised by some strange shit, that certanly WILL happen, if you do not fix it.
	...
	if((radiation_repository.get_rads_at_turf(get_turf(src)) - own_rads) > 0)
		reaction_rads += (radiation_repository.get_rads_at_turf(get_turf(src)) - own_rads)
	if (reaction_rads < 0)
		reaction_rads = 0

	if(reactants.len)
		var/list/produced_reactants = new /list(0)
		var/list/allreactions = decls_repository.get_decl(/decl/nuclear_reaction)
