...
/obj/machinery/control_rod  // Self-explanatory, keeps reaction stable
	name = "Control rods"
	desc = "Control rods, keeping the reactor from melting down."
	...
	var/target = 0
	var/speed = 0.1
	var/catched_rad_resistance = 500
	var/health = 200
	var/load = 0
	var/nocontrol = 0
	var/id
...
/obj/machinery/control_rod/update_icon()
	GLOB.control_rods -= src
	return ..()
...
/obj/machinery/control_rod/attackby(obj/item/weapon/W, mob/user)
	if(health > 0)
		if(isMultitool(W))
			var/new_id = input("Enter a new identification tag.", "Control rod", id) as null|text
