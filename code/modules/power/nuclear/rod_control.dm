...
/obj/machinery/computer/reactor_control/ui_interact(mob/user)
	ui_interact(user)
	return TRUE
...
/obj/machinery/computer/reactor_control/attackby(var/obj/item/W, var/mob/user)
	if(isMultitool(W))
		var/new_ident = input("Enter a new ID.", "Rod Control", id) as null|text
...
/datum/nano_module/rmon/proc/FindDevices()
	...
	for(var/obj/machinery/power/nuclear_rod/I in GLOB.nrods)
		if(I.id && (I.id == id)) //&& (get_dist(src, I) < 50))
			known_rods += I
