...
//Regular jobs
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		if(isliving(M))
			var/mob/living/L = M
			var/status
			religion_description = "<font color='red'><b>[L.religion]</b></font>."
			switch (M.stat)
				if (0) status = "Alive"
		...
		to_chat(src.owner, "Mob type = [M.type]; Gender = [gender_description] Religion = [religion_description] Damage = [health_description]")
